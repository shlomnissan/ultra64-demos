...
{"Dump RDP DL", MT_ACTION,   0,      0,0,0,0,               0,0,0,       FONT_COL},
{"Dump RSP DL", MT_ACTION,   0,      0,0,0,0,               0,0,0,       FONT_COL},
{"Dump RSP raw",MT_ACTION,   0,      0,0,0,0,               0,0,0,       FONT_COL},
...
#define NUMMODES 4
	...
	if (mainmenu[MM_VIEWSCALE].val < 1.0 && !((int)dumpmenu[DM_RDP].val) &&
						!((int)dumpmenu[DM_RSP].val) &&
						!((int)dumpmenu[DM_RSPRAW].val)) {
		gDPPipeSync(glistp++);
	 ...
	 */
	if (dumpmenu[DM_RDP].val ==1.0) {
		dumpmenu[DM_RDP].val =0.0;
		rdp_DRAM_io=1;
	}
	 ...
	 */
	if (dumpmenu[DM_RSP].val ==1.0) {
		dumpmenu[DM_RSP].val =0.0;
		guParseGbiDL((u64 *) dynamicp->glist, 0, GU_PARSEGBI_SHOWDMA | 
								GU_PARSEGBI_ALLMTX);
		osSyncPrintf("\n\n==========DONE WITH DUMP==========\n\n");
	}
	if (dumpmenu[DM_RSPRAW].val ==1.0) {
		int i;
		dumpmenu[DM_RSPRAW].val =0.0;
		guParseGbiDL(theadp->t.data_ptr, theadp->t.data_size,
		    GU_PARSEGBI_DUMPONLY);
		for (i=0; i<100; i++) osSyncPrintf("=====================================\n");
		osSyncPrintf("\n\n==========DONE WITH DUMP==========\n\n");
	}
