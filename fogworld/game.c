...
MenuItem viewmenu[] = {
	...
	55, 255, 155, 0xff},
	{"Dump RSP -rspsim +parse",MT_ADDVALS,(MenuItem *) bStr, 0,1,-1,1,
	55, 255, 155, 0xff},
	{"Dump RDP",           MT_ADDVALS,(MenuItem *) bStr, 0,1,-1,1,
	55, 255, 155, 0xff},
...
game(void)
{
	 ...
	 * Draw Text
	 */
        if (viewmenu[VM_DUMPRDP].val == 0.0  &&
	    viewmenu[VM_DUMPRSP].val == 0.0) {
	...
	if (viewmenu[VM_DUMPRSP].val != 0.0) {
	    if (viewmenu[VM_DUMPRSP].val == 1.0)
	        guParseGbiDL(theadp->t.data_ptr,0,
			GU_PARSEGBI_SHOWDMA|GU_PARSEGBI_ALLMTX);
	    if (viewmenu[VM_DUMPRSP].val == -1.0)
