...
/*
 * buffers used by dram and fifo microcode only
 */
#if __GNUC__ /* { */
...
OSTask          taskHeader =
{
	...
	&(rdp_output[0]),	/* output buffer (dram & fifo ucode only) */
	&rdp_output_len,	/* pointer used to return output buf length */
...
MenuItem dumpmenu[] = {
	...
	{"Main menu",	       MT_UPMENU,    0,0,0,0,0,
	5, 255, 155, 0xff},
	{"Save Terrain",       MT_ADDVALS,(MenuItem *) noStr, 0, 1,  -1,  1,
	5, 255, 155, 0xff},
	{"Load Terrain",       MT_ADDVALS,(MenuItem *) noStr, 0, 1,  -1,  1,
	5, 255, 155, 0xff},
	{"Save Mode : ",       MT_ADDVALS,(MenuItem *) svStr, 2, 1,   0,  2,
	5, 255, 155, 0xff},
	{"dump RDP surface",   MT_ADDVALS,(MenuItem *) noStr, 0, 1,  -1,  1,
	5, 255, 155, 0xff},
	{"dump RDP contours",  MT_ADDVALS,(MenuItem *) noStr, 0, 1,  -1,  1,
	5, 255, 155, 0xff},
	{"parse RSP surface",  MT_ADDVALS,(MenuItem *) noStr, 0, 1,  -1,  1,
	5, 255, 155, 0xff},
	{"",                   MT_END,       0,0,0,0,0,
	55, 255, 155, 0xff},
};
...
MenuItem mainmenu[] = {
	...
	55, 255, 155, 0xff},
	{"Dump Menu",          MT_MENU,    dumpmenu, 0,0,0,0,
	55, 255, 155, 0xff},
...
PRINTF("returned from makeground\n");
	 ...
	 * menu commands 
	 */
	if (dumpmenu[DM_SAVE].val != 0.0) {
	    cmd[0]=svChar[(int)dumpmenu[DM_SAVEMODE].val];
	    osWriteHost(cmd,4);
	    saveterrain();
	    dumpmenu[DM_SAVE].val = 0;
	}
	if (dumpmenu[DM_LOAD].val != 0.0) {
	    cmd[0]='l';
	    osWriteHost(cmd,4);
	    loadterrain();
    	    selx=dimx/2;
    	    selz=dimz/2;
	    dumpmenu[DM_LOAD].val = 0;
	}
...
#if 0
	if (modemenu[OM_LOOKATC].val == 1.0) {
	    pos[0] = -(*groundvtx + selx*dimz + selz)->n.ob[0] * scale;
	    pos[1] = -(*groundvtx + selx*dimz + selz)->n.ob[1] * scale;
	    pos[2] = -(*groundvtx + selx*dimz + selz)->n.ob[2] * scale;
	}
#endif
	...
	if (modemenu[OM_LOOKATC].val == 1.0) {
	    ...
	    guPositionF(mm,
		rotb, rota, 0.0, scale, pos[0], pos[1], pos[2]);
	    atx = (*groundvtx + selx*dimz + selz)->n.ob[0] * mm[0][0] +
	          (*groundvtx + selx*dimz + selz)->n.ob[1] * mm[0][1] +
	          (*groundvtx + selx*dimz + selz)->n.ob[2] * mm[0][2] +
	                                                     mm[0][3];
	    aty = (*groundvtx + selx*dimz + selz)->n.ob[0] * mm[1][0] +
	          (*groundvtx + selx*dimz + selz)->n.ob[1] * mm[1][1] +
	          (*groundvtx + selx*dimz + selz)->n.ob[2] * mm[1][2] +
	                                                     mm[1][3];
	    atz = (*groundvtx + selx*dimz + selz)->n.ob[0] * mm[2][0] +
	          (*groundvtx + selx*dimz + selz)->n.ob[1] * mm[2][1] +
	          (*groundvtx + selx*dimz + selz)->n.ob[2] * mm[2][2] +
	                                                     mm[2][3];
	} else {
	    atx = aty = atz = 0.0;
	}
...
*/
    ...
    gDPSetCycleType(glistp++, G_CYC_1CYCLE);
      ...
      if (dumpmenu[DM_SURFDP].val == 0.0 &&
		dumpmenu[DM_LINEDP].val == 0.0) {
	/* 
	 * Draw some text 
	 */
	font_init( &glistp );
	...
	if (rdp_DRAM_io) {
	    ...
	    if (modemenu[OM_NEARCLIP].val == 1.0) {
	        theadp->t.ucode = (u64 *) gspFast3D_dramTextStart;
	        theadp->t.ucode_data = (u64 *) gspFast3D_dramDataStart;
	    } else {
	        theadp->t.ucode = (u64 *) gspF3DNoN_dramTextStart;
	        theadp->t.ucode_data = (u64 *) gspF3DNoN_dramDataStart;
	    }
	} else {
	    /*
	     ...
	     */
	    if (modemenu[OM_NEARCLIP].val == 1.0) {
	        theadp->t.ucode = (u64 *) gspFast3DTextStart;
	        theadp->t.ucode_data = (u64 *) gspFast3DDataStart;
	    } else {
	        theadp->t.ucode = (u64 *) gspF3DNoNTextStart;
	        theadp->t.ucode_data = (u64 *) gspF3DNoNDataStart;
	    }
	}
	...
	if (dumpmenu[DM_SURFSPP].val != 0.0) {
	    guParseGbiDL(theadp->t.data_ptr,0,
				GU_PARSEGBI_SHOWDMA|
				GU_PARSEGBI_ALLMTX|
				GU_PARSEGBI_DUMPONLY);
	    dumpmenu[DM_SURFSPP].val = 0.0;
	}
	...
	{
	int count=0;
	while (osRecvMesg(&rspMessageQ, NULL, OS_MESG_NOBLOCK) == -1) {
		if (++count==1000000) {
			PRINTF("Failed in triangle RSP wait\n");
			RCP_STAT_PRINT;
guParseGbiDL(theadp->t.data_ptr,0,GU_PARSEGBI_SHOWDMA|GU_PARSEGBI_ALLMTX|GU_PARSEGBI_DUMPONLY);
		}
	}
	}
#endif
	...
	/*
	 * if using DRAM microcode, must tell RDP to start parsing
	 * RDP display list after RSP has finished creating it
	 */
	if (rdp_DRAM_io) {
	    if (dumpmenu[DM_SURFDP].val != 0.0) {
	        guParseRdpDL(rdp_output, rdp_output_len, 0);
	        dumpmenu[DM_SURFDP].val = 0; 
	    }
	...
	{
	int count=0;
	while (osRecvMesg(&rdpMessageQ, NULL, OS_MESG_NOBLOCK) == -1) {
		if (++count==10000) {
			PRINTF("Failed in triangle RDP wait\n");
			RCP_STAT_PRINT;
		}
	}
	}
	...
	if (rdp_DRAM_io) {
	    theadp->t.ucode = (u64 *) gspLine3D_dramTextStart;
	    theadp->t.ucode_data = (u64 *) gspLine3D_dramDataStart;
	} else {
	    theadp->t.ucode = (u64 *) gspLine3DTextStart;
	    theadp->t.ucode_data = (u64 *) gspLine3DDataStart;
	}
...
/*guParseGbiDL(theadp->t.data_ptr,0,GU_PARSEGBI_SHOWDMA|GU_PARSEGBI_ALLMTX|GU_PARSEGBI_DUMPONLY);*/
	...
	while (osRecvMesg(&rspMessageQ, NULL, OS_MESG_NOBLOCK) == -1) {
		if (++count==1000000) {
			PRINTF("Failed in line RSP wait\n");
			RCP_STAT_PRINT;
		}
	}
	}
	...
	if (rdp_DRAM_io) {
	    if (dumpmenu[DM_LINEDP].val != 0.0) {
	        guParseRdpDL(rdp_output, rdp_output_len, 0);
	        dumpmenu[DM_LINEDP].val=0;
	    }
	    osDpSetNextBuffer(&(rdp_output[0]), (u32) rdp_output_len);
	}
	...
	while (osRecvMesg(&rdpMessageQ, NULL, OS_MESG_NOBLOCK) == -1) {
		if (++count==10000) {
			PRINTF("Failed in line RDP wait\n");
			RCP_STAT_PRINT;
		}
	}
	}
