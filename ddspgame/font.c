...
text_sprite( Sprite *txt, char *str, Font *fnt, int xlen, int ylen )
{
...
#ifdef _EMULATOR
    emPrintf("Converting <%s> to a %d x %d sprite\n", str, xlen, ylen );
#endif
	...
	for(x=0; x<xlen; x++, i++, ci++) {
		...
		txt->nbitmaps = i;
#ifdef _EMULATOR
		emPrintf("\tEarly exit x,y=(%d,%d) num=%d\n", x,y,i );
#endif
		return;
	    };
	    if( str[ci] == '\n' ) {
#ifdef _EMULATOR
		emPrintf("\t<CR> at x,y=(%d,%d) num=%d\n", x,y,i );
#endif
		bm[i]      = fnt->bitmaps[0];
		bm[i].buf = NULL;
		ci--;
...
#ifdef _EMULATOR
		emPrintf("\t<%c>(%d) at x,y=(%d,%d) num=%d\n", str[ci],indx-fnt->index_string,x,y,i );
#endif
		bm[i]      = fnt->bitmaps[indx-fnt->index_string];
	    } else {
#ifdef _EMULATOR
		emPrintf("\t<%c>(unknown) at x,y=(%d,%d) num=%d\n", str[i],x,y,i );
#endif
		bm[i]      = fnt->bitmaps[0];
		bm[i].buf = NULL;
	    };
...
#ifdef _EMULATOR
    emPrintf("\tRegular exit x,y=(%d,%d) num=%d\n", x,y,i );
#endif
    return;
}
...
/* This function has been heavily modified to allow for
 *   the variable sizes of ASCII font characters.
 *   I have continued to use the DDROM fonts to show
...
jfont_show_string( Gfx **glistp, u16 mes )
{
  ...
  int		addr = kaddr[mes];
  int		tmpDX, tmpDY, tmpCY;
  int		i;

  gxp = *glistp;

  for (i=0; message[mes][i]; i++) 
    totalWidth += sizeDB[(int)(message[mes][i])-32].dx + 1;

  charXpos = (SCREEN_WD - totalWidth)/2;
  charYpos = font_ypos;
  for (i=0; message[mes][i]; i++, addr+=128) {
	tmpDX = sizeDB[(int)(message[mes][i])-32].dx;
	tmpDY = sizeDB[(int)(message[mes][i])-32].dy;
	tmpCY = sizeDB[(int)(message[mes][i])-32].cy;
    gDPLoadTextureTile_4b(gxp++, &kbuffer[addr], G_IM_FMT_I,  
		(tmpDX+1)&~1, tmpDY, 0, 0, (tmpDX-1)<<G_TEXTURE_IMAGE_FRAC,
		(tmpDY-1)<<G_TEXTURE_IMAGE_FRAC, 0, 
		G_TX_CLAMP, G_TX_CLAMP, G_TX_NOMASK, G_TX_NOMASK,
		G_TX_NOLOD, G_TX_NOLOD);
	gSPTextureRectangle(gxp++, charXpos<<2, (charYpos - tmpCY)<<2,
		(charXpos + tmpDX)<<2, (charYpos - tmpCY + tmpDY)<<2, 
		0, 0, 0, 0x400, 0x400);
	charXpos += tmpDX + 1;	
  }
